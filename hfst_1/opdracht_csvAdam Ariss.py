...
for index, rij in enumerate(eruptions_ll):#doorloop alle index en rijen en tel de eruptions_ll
    if index == 0:# als de index gelijk is aan 0 dan
        ...
        rij[3] = int(rij[0])**4
        eruptions_ll_verwerkt.append([rij[0],rij[1],rij[2],rij[3]])#voeg de rijen  toe aan eruptions_ll_verwerkt
        print(rij)#print de doorgelopen rijen
fp = open( "toetsAdamAriss.csv", "w", newline="")# open een bestand genaamd toets.csv en we gaan erin writen de "" wordt gekenmerkt door een nieuwe lijn
