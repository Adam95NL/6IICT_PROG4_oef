...
fp = open( "experiment10in.csv", "r" )#open het csv bestand met lees modus
...
lijst_tijd = []
lijst_snelheid = []
for index,rij in enumerate(csv_reader): # doorloop rij in csv_reader
    if index == 0:
        pass
    else:
        lijst_tijd.append(float(rij[0]))#voeg rij toe aan eruptions_ll
        lijst_snelheid.append(float(rij[1]))#voeg rij toe aan eruptions_ll
...
plt.plot(lijst_tijd, lijst_snelheid, 'g')                                  # Plot de rotatie ten opzichte van de tijd.
plt.plot(lijst_tijd, [gem_rot_1]*len(lijst_tijd), ':g')                     # Plot de gemiddelde rotatie ten opzichte van de tijd.
